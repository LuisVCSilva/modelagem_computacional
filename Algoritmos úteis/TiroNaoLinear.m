...
function [TiroNaoLinear] = main(argv)
 %y'' = 0.5*(1-y'^2-y*sin(x))
 F = inline('0.5*(1-(z^(2))-y*sin(x))') %funcao original
 fl = inline('-0.5*sin(x)') %derivada de y
 fll = inline('-z') %derivada de z
 ...
 subIntervalos = 20;%ou 4 para h=0.25 e 2 para h=0.5
 _flag = 0;
